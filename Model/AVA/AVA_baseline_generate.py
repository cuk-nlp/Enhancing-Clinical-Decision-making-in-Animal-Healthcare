import os
# os.environ["HF_HOME"] = "" your huggingface cache path (select)
import torch
import nltk
nltk.data.path.append(r"")  # your 'mynltk' folder path 
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    # device_map={"": "cuda:1"}, # Default: 'auto'
...
def extract_symptoms_from_memo_llm(memo, batch_size=5):
    ...
    for i in range(0, len(sentences), batch_size):
        ...
        for sentence in batch_sentences:
            prompt = f""" # Use the prompt exactly as stated in the paper
            """
        ...
        for output in outputs:
            ...
            index = decoded.find("**Actual Output**:")
            short_response = decoded[index + len("**Actual Output**:"):].strip() if index != -1 else decoded.strip()
...
def process_memo(row):
    ...
    predicted_symptoms = extract_symptoms_from_memo_llm(memo, batch_size=5)
   
    initial_diseases = find_related_diseases(predicted_symptoms)
...
true_col = df_memo['Selected symptoms']
pred_col = df_results['Extracted symptoms']
...
true_labels_list = true_col.apply(preprocess).tolist()
pred_labels_list = pred_col.apply(preprocess).tolist()

def calculate_accuracy(true_labels, pred_labels):
    if not true_labels:
        return 1.0 if not pred_labels else 0.0
    matching_count = len(set(true_labels) & set(pred_labels))
    total_true_labels = len(set(true_labels))
    return matching_count / total_true_labels if total_true_labels > 0 else 0
...
df_results['Accuracy'] = accuracy_scores
mean_accuracy = np.mean(accuracy_scores)

logging.info("Sample Accuracy (symptoms): %s", accuracy_scores)
logging.info("Mean Accuracy (symptoms): %.3f", mean_accuracy)
